...
const UploadFile = ({ register, errors, setValue, getValues, name, label, video = false, viewData = null, editData = null }) => {
    ...
    const { getRootProps, getInputProps, isDragActive } = useDropzone({
        accept: !video ? { "image/*": [".jpg", ".png", ".svg", ".jpeg"] } : { "video/*": [".mp4"] },
        onDrop,
    })
            ...
            <div className={`${isDragActive ? "bg-richblack-600" : "bg-richblack-700"} flex min-h-[250px] cursor-pointer items-center justify-center rounded-md border-2 border-dotted border-richblack-500`}>
                {
                    previewSource ? (
                        <div className="flex w-full flex-col p-6">
                            {
                                !video ? (
                                    <img src={previewSource} alt='Source' className="h-full w-full rounded-md object-cover" />
                                ) : (
